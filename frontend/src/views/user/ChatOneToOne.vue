    ...
    <div class="chat-container" id="chat-page" ref="chatPage">
        <div class="users-list">
            <div class="users-list-container">
                <el-skeleton v-if="!dataLoaded" :rows="12" animated style="padding: 10px 30px 10px 30px; height: 100%; overflow: auto;" />
                <ul id="connectedUsers" ref="connectedUsersList"></ul>
            </div>
        </div>
        ...
        <div class="chat-area">
            ...
            <div class="chat-area" id="chat-messages" ref="chatArea"></div>

            <form id="messageForm" name="messageForm" @submit.prevent="sendMessage">
                <div class="message-input">
                    <input autocomplete="off" type="text" id="message" placeholder="Type your message..." v-model="messageInput">
                    <button>
                        <el-icon size="20px">
                            <Promotion />
                        </el-icon>
...
async function findAndDisplayConnectedUsers() {
    let alluser = connectedUsers.value.filter(user => user.username !== username.value);
...
function displayMessage(senderId, content) {
    ...
    messageContainer.classList.add('message', senderId === username.value ? 'sender' : 'receiver');
...
function sendMessage() {
    ...
    if (messageContent && stompClient.value) {
        ...
        displayMessage(username.value, messageContent);
        messageInput.value = '';
    }
    chatArea.value.scrollTop = chatArea.value.scrollHeight;
}
