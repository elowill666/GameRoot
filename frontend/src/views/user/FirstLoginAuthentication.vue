...
const validateCode = (rule, value, callback) => {
    ...
    } else if (value != generateIdentifyCode.value) {
...
const generateIdentifyCode = computed(() => {
    ...
    for (let i = 0; i < state.count; i++) {
        code += state.valicodes[randomNum(0, state.valicodes.length - 1)];
    }
    return code;
});
...
const submitForm2 = async (formEl) => {
    ...
    formEl.validate(async (valid) => {
        if (valid) {
            ...
            const res = await axios.get(`${url}/user/emailAuthenticate`, { withCredentials: true });
