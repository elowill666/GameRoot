...
async function gochangepassword() {
    try {
        const res = await axiosObj.get(`${url}/user/getuser/${form.username}`);
        if (res.data === '使用者不存在') {
            Swal.fire({
                title: 'Error!',
                text: res.data,
            ...
            const res = await axiosObj.post(`${url}/user/changepassword`, data);
            if (res.data === '密碼更新成功') {
                await Logout();
            }
        }
...
onMounted(async () => {
    try {
        const res = await axiosObj.get(`${url}/user/profile`);
